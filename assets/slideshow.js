...
export class Slideshow extends Component {
  ...
  async select(input, event, options = {}) {
    ...
    for (const slide of this.refs.slides) {
      if (slide.hasAttribute('reveal')) {
        ...
        this.#setSlideVisibility(slide, false);
      }
    }
    ...
    let requestedIndex = (() => {
      ...
      if ('id' in input) {
        ...
        if (requestedSlide.hasAttribute('hidden')) {
          ...
          this.#setSlideVisibility(requestedSlide, true);
        }
    ...
    this.#setSlideVisibility(slide, true);
  ...
  #setupSlideshowWithoutControls() {
    ...
    if (this.refs.slides?.[0]) {
      this.#setSlideVisibility(this.refs.slides[0], true);
    }
  }
  ...
  #updateVisibleSlides() {
    ...
    scheduler.schedule(() => {
      ...
      slides.forEach((slide) => {
        ...
        this.#setSlideVisibility(slide, isVisible);
      });
    });
